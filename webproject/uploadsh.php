		...
		if($fileError === 0){
			if($fileSize < 200000 ){
				...
				if(empty($_imgtitlee)|| empty($_imgdesc)){
					header("Location:shirt.php?upload=empty");
					exit();
					...
					if(!mysqli_stmt_prepare($stmt,$sql)){
						...
						$sql = "INSERT INTO shirt ( titlegallery, descgallery, imgfullnamegallery, ordergallery) VALUES (?,?,?,?);";
						...
						if(!mysqli_stmt_prepare($stmt,$sql)){
							...
							header("Location:shirt.php?upload=success");
						}
